...
例えば、同じ人物の名刺を別の支援員も持っている場合に、名刺を二重登録するのではなく、既存レコードの担当支援員欄へ追加する仕様でよいかを支援員に確認しました。
既存の担当支援員が「佐々木」、追加する担当支援員が「鈴木」の場合は、「佐々木,鈴木」として両方の情報を残します。名刺情報の重複を抑えながら、どの支援員がその連絡先を担当しているかを確認できるようにしました。
...
開発・動作確認にはダミーデータを使用し、実際の名刺データはフォーム完成後に取り込む方針としました。自分のPCに個人情報を保存する期間をできるだけ短くするため、機能の実装だけでなく、実データを扱い始めるタイミングにも配慮しました。
...
Access Runtimeでの利用も想定し、メニュー画面から登録・検索・編集・削除・インポートへ進める構成にしました。

開発環境で動くことだけでなく、利用先で必要な画面を開き、一連の操作を進められるかという観点から、フォーム間の移動方法を確認・調整しました。

### 安全性・利便性を考慮した操作設計

機能が動くことだけでなく、利用者の誤操作を防ぐことや、作業を中断せずに進められることも意識して仕様を検討しました。

**安全性：既存データの誤変更を防ぐ**

検索画面を閲覧用にし、新規登録フォームでは既存レコードを編集できないようにすることを検討しました。連絡先を探しているときや、新しい名刺を登録しているときに、誤って別の既存データを書き換えてしまうリスクを減らすためです。

**利便性：1件をキャンセルしても、残りの取込を続けられるようにする**

重複候補画面でキャンセルした後、一括インポートが続行されない問題に気づき、動作の見直しを求めました。「その1件を取り込まない」と「取込全体を終了する」を区別し、登録を見送るデータがあっても、残りの処理を続けられる仕様を考えました。
